...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('[data-action="release"]').forEach(btn => {
        btn.addEventListener('click', async function() {
            ...
            const confirmed = confirm(`Release certificate for ${student}?\n\nThis will move it to the Cleared Students list.`);
            if (!confirmed) return;

            this.disabled = true;
            this.textContent = 'Releasing…';

            try {
                const formData = new FormData();
                formData.append('certId', this.dataset.certId);
